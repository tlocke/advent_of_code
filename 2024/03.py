    ...
    for c in line:
        if in_func:
            if in_prefix:
                ...
                if c == ")":
                    total += int("".join(first_num)) * int("".join(second_num))
                    in_func = False
                elif c in "0123456789":
                    if len(second_num) == 3:
                        in_func = False
...
def part_two(inpu):
    FUNCS = {"mul": 2, "do": 0, "don't": 0}

    funcs = []

    for line in inpu.splitlines():
            ...
            if in_prefix:
                if c == "(":
                    for fname in FUNCS.keys():
                        if fname == "".join(prefix[-1 * len(fname) :]):
                            in_arg = True
                            in_prefix = False
                            func_name = fname
                            arg = []
...
print(inp_test)
test_total = part_two(inp_test)

assert test_total == 48, test_total

actual_total = part_two(inp)
